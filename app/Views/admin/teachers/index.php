...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-body-tertiary small mb-0">Kelola staf pengajar sekolah</p>
    </div>
    <a href="/admin/teachers/new" class="btn btn-primary rounded-pill px-3">
            ...
            <tbody>
                ...
                <tr data-id="<?= $item['id'] ?>" draggable="true">
                    ...
                    <td>
                        ...
                        <?php else: ?>
                            <span class="d-inline-flex align-items-center justify-content-center rounded-circle bg-body-tertiary" style="width:40px;height:40px">
                                <i class="fas fa-user text-body-tertiary"></i>
                            </span>
                        <?php endif; ?>
                    </td>
                    <td class="fw-medium"><?= esc($item['name']) ?></td>
                    <td class="text-body-secondary"><?= esc($item['role'] ?? '-') ?></td>
                    <td class="text-body-secondary small"><?= esc($item['experience'] ?? '-') ?></td>
                    <td class="text-body-secondary small"><?= esc($item['education'] ?? '-') ?></td>
                    <td><span class="badge bg-light text-body-tertiary border"><?= $item['sort_order'] ?? 0 ?></span></td>
                    <td>
                        <div class="d-flex gap-1">
                            <a href="/admin/teachers/edit/<?= $item['id'] ?>" class="btn btn-sm btn-outline-warning border-0" title="Edit">
                            ...
                            <div class="modal-dialog modal-sm modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-body">
                                        Yakin ingin menghapus "<strong><?= esc($item['name']) ?></strong>"?
                                    </div>
                                    <div class="modal-footer border-0 pt-0">
                                        <button class="btn btn-sm btn-secondary" data-coreui-dismiss="modal">Batal</button>
                                        <a href="/admin/teachers/delete/<?= $item['id'] ?>" class="btn btn-sm btn-danger">Hapus</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <?php endforeach; ?>
                <?php if (empty($items)): ?>
                <tr><td colspan="9" class="text-center py-5 text-body-tertiary">Tidak ada guru ditemukan.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
