...
<div class="d-flex justify-content-between align-items-center mb-4">
    <div>
        ...
        <p class="text-body-tertiary small mb-0">Kelola program akademik</p>
    </div>
    <a href="/admin/programs/new" class="btn btn-primary rounded-pill px-3">
...
<div class="card shadow-sm border-0 rounded-3">
    <div class="table-responsive">
        <table class="table table-crud table-hover align-middle mb-0" id="programs-table">
            <thead>
                <tr>
            ...
            <tbody>
                ...
                <tr data-id="<?= $item['id'] ?>" draggable="true">
                    ...
                    <td><i class="<?= esc($item['icon'] ?? 'fas fa-circle') ?> fa-lg"></i></td>
                    <td class="fw-medium"><?= esc($item['title']) ?></td>
                    <td class="text-body-secondary small"><?= esc(substr($item['description'] ?? '', 0, 80)) . (strlen($item['description'] ?? '') > 80 ? '...' : '') ?></td>
                    <td><span class="badge bg-light text-body-tertiary border"><?= $item['sort_order'] ?? 0 ?></span></td>
                    <td>
                        <div class="d-flex gap-1">
                            <a href="/admin/programs/edit/<?= $item['id'] ?>" class="btn btn-sm btn-outline-warning border-0" title="Edit">
                            ...
                            <div class="modal-dialog modal-sm modal-dialog-centered">
                                <div class="modal-content">
                                    ...
                                    <div class="modal-footer border-0 pt-0">
                                        ...
                                        <a href="/admin/programs/delete/<?= $item['id'] ?>" class="btn btn-sm btn-danger">Hapus</a>
                                    </div>
                                </div>
                            </div>
                        </div>
                ...
                <?php endforeach; ?>
                <?php if (empty($items)): ?>
                <tr><td colspan="7" class="text-center py-5 text-body-tertiary">Tidak ada program ditemukan.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
